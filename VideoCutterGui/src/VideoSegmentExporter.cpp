...
ExportResult VideoSegmentExporter::Export(const ExportSettings& settings) {
    ...
    if (ec) {
        result.error = L"Cannot create output folder: " + std::wstring(ec.message().begin(), ec.message().end());
        Log(result.error);
        return result;
    }
    ...
    if (ec) {
        result.error = L"Cannot create temp folder: " + std::wstring(ec.message().begin(), ec.message().end());
        Log(result.error);
        return result;
    }
